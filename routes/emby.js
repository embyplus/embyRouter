...
async function checkEmbyUserLimit(embyId, itemId) {
  ...
  recordArr = recordArr.filter(
    (entry) => now - entry.firstTime < ONE_HOUR_IN_SECONDS
  );
  ...
  await embyLimitKV.put(embyId, JSON.stringify(recordArr), {
...
function getApiKeyOrThrow(req) {
  ...
  if (apiKey == null) {
    // 检查header
    apiKey = req.headers["x-emby-token"]
...
async function handleStream(req, res) {
  try {
    ...
    if (isLimited) {
      telegramNotify(
        `Emby user ${embyId} reached the limit of 50 times per hour.`
      ).catch(console.error);
      await redirectToDefault(res);
      return;
    }
    ...
    switch (embyRouter.type.toUpperCase()) {
      case ROUTER_TYPE_REAL_PATH:
        ...
        const playbackInfo = await userEmbyClient.getPlaybackInfo(
          itemId,
          embyId,
          mediaSourceId
        );
